...
(
        sleep 90s
        find ~ -name *.bak* -mtime +7 -exec rm {} \;
)

# Nettoyage du cache gradle
(
        sleep 100s
        find .gradle/caches/*/scripts/ -mtime +15 -exec rm -rf {} \;
)
